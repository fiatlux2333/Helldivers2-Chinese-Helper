...
2. 下载并双击 `Helldivers 2 中文助手_0.5.3_x64-setup.exe`
...
仓库根目录的 `release/` 不保存历史安装包；每个版本的安装程序统一附在 GitHub Releases 中，避免源码仓库继续累积旧二进制。
...
- 🔎 **聊天翻译** — 内置 Windows 离线中英混合 OCR，按聊天行拆分并追踪增量消息；短消息不会因 OCR 漏掉重复说话人而被合并，只把新出现的消息正文逐条送往现有 OpenAI 兼容翻译接口
