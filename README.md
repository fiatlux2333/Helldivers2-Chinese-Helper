...
| 权限 | 助手与游戏必须同一权限级别；优先都普通运行，若游戏管理员运行则助手也必须管理员运行 |
| 技术栈 | Vue 3 + TypeScript + pnpm · Rust · Tauri 2 |

## 📖 介绍

HD2CN 提供独立的中文 IME 输入、显式直发、中译英、快捷喊话、战备一键触发，以及基于 Windows OCR 的游戏聊天截图翻译。
...
- **战备模块** — 使用预设或手动配置方向序列，支持 WASD / 方向键输入和全局热键触发
- **输入法保护** — 游戏前台自动保持 CapsLock 游戏态，侧栏输入时临时恢复中文输入，降低 HD2 中文 IME 卡键风险
...
2. 下载并双击 `Helldivers 2 中文助手_0.6.1_x64-setup.exe`
...
7. 侧栏发送、取消和快捷喊话前会先释放侧栏焦点，并同步切回游戏态 CapsLock，减少输入法残留按键
```
...
```text
1. 在助手主界面关闭「中文侧栏」
2. 游戏使用窗口化 / 无边框，点击「捕获 HD2」并按提示切回游戏
3. 捕获完成后回到助手输入中文，选择「中文直发」或「中译英」
4. 按 `Enter` 会自动打开游戏聊天框并直接发送；按 `Ctrl+Enter` 仅填入并回游戏检查
5. 首次使用聊天翻译时校准区域；之后按截图翻译热键读取聊天
6. 主界面填入同样使用 Unicode SendInput 和游戏态 CapsLock 保护
```
...
7. “英译中结果位置”可选择默认进入“聊天翻译”页，或留在游戏内透明 HUD 显示最近聊天译文；HUD 支持按游戏可用区域自定义水平和垂直位置
...
在“战备”页可从预设库加入常用战备，也可手动配置方向序列、菜单键、菜单模式和按键节奏。方向按键可按游戏内设置选择 WASD 或方向键。战备全局热键默认允许组合键和功能键；“允许数字单键热键”默认关闭，开启后可绑定主键盘数字和小键盘数字，字母、Space、Tab、Ctrl、Alt、Shift 单键仍禁止。
...
- 🧷 **CapsLock 防卡输入法** — 游戏前台保持 CapsLock 游戏态，侧栏/助手输入时临时关闭，Unicode 发送前后会同步恢复游戏态，降低 HD2 把中文输入法指令误判为游戏操作的概率；不会强制把 Windows 键盘布局切成英文
...
- ✂️ **文本清理与注入** — 剔除控制字符、CR/LF/NUL 等；单条上限 100 字；默认使用逐字符 Unicode `SendInput`，GBK Alt 数字码已从普通设置中停用
- 🔁 **连续会话** — 填入成功保留目标，支持连续多句
- 🌐 **中译英** — API 成功并通过最终长度校验后才进入注入链路
- 🔎 **聊天翻译** — 内置 Windows 离线中英混合 OCR，按聊天行拆分并追踪增量消息；短消息不会因 OCR 漏掉重复说话人而被合并，只把新出现的消息正文逐条送往现有 OpenAI 兼容翻译接口，并可选择在“聊天翻译”页或游戏内透明 HUD 展示结果
- 📚 **HD2 官方词库提示词** — 英译中优先使用官方简体中文译名；中译英优先使用对应英文原名，并且只把当前消息命中的术语候选加入提示词
...
- 🎯 **一键战备** — 战备预设、手动方向序列、WASD / 方向键切换、全局热键和可选数字单键触发
- 🔽 **不抢焦点** — 主界面发送后保持最小化；右侧输入栏发送或取消成功后自动隐藏
- ⎋ **稳定取消** — 侧栏取消会等待按键释放、恢复并确认 HD2 前台稳定后再发送 Esc，失败时保留注入报告用于排查
...
3. **权限一致**  
   Windows 会阻止低权限助手控制高权限游戏。请关闭游戏和助手后重新打开，优先都普通运行；如果游戏必须管理员运行，助手也必须管理员运行。

4. **输入法保护不是游戏 Bug 的根治**
   助手会用 CapsLock 策略降低 HD2 中文输入法卡键风险，但 HD2 自身仍可能错误处理中文 IME 指令。若出现按键异常，请先切出游戏再切回，或关闭中文输入法后重试。
...
- 默认中文填入路径使用逐字符 `KEYEVENTF_UNICODE` `SendInput`；GBK Alt 数字码保留在内部兼容代码中，但普通设置会迁移到 Unicode（见 `src-tauri/src/platform/windows/injector.rs`）
- 游戏前台与文字注入前后会调用 CapsLock 游戏态保护；侧栏/助手输入时临时关闭 CapsLock，退出游戏或禁用功能时恢复用户原始状态（见 `src-tauri/src/platform/windows/game_monitor.rs`）
- 侧栏取消会在按键释放、目标恢复和前台稳定确认后发送 Esc，并在失败时把 `InjectionReport` 带回前端
...
| 项目 | 状态 |
| --- | --- |
| Vue 面板 / 类型检查 / 前端测试 | ✅ 通过（103/103） |
| Rust 核心测试 | ✅ 通过（80/80） |
| Rust clippy / 格式检查 | ✅ 通过 |
| 前端生产构建 | ✅ 通过 |
| 记事本探针 | 待按当前版本复测 |
| HD2 窗口化填字 / 侧栏 / HUD / Esc 取消 | 待按当前版本复测 |
| NSIS 安装包 | 0.6.1 发布包将在本次构建生成 |
