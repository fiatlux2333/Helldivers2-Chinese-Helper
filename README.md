...
- **双向提示词** — 游戏聊天英译中和输入消息中译英可分别自定义，留空时使用内置 HD2 玩家词库
...
2. 下载并双击 `Helldivers 2 中文助手_0.3.0_x64-setup.exe`
...
3. 在助手输入中文，选择「中文直发」或「中译英」
4. 按 `Enter` 直接发送；按 `Ctrl+Enter` 仅填入并回游戏检查
5. 下一句前在游戏中重新打开聊天框，再用唤回热键打开助手
...
2. 按需编辑“游戏聊天英译中提示词”和“输入消息中译英提示词”；留空时使用内置 HD2 术语词库
3. 捕获 HD2 后进入“聊天翻译”，点击“校准区域”
4. 助手最小化并截取游戏客户区；在预览图上拖选聊天消息区域并保存
5. 点击“读取并翻译”或使用截图翻译热键；相同的旧聊天行不会重复提交 API
