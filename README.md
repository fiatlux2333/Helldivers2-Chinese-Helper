...
2. 下载并双击 `Helldivers 2 中文助手_0.6.0_x64-setup.exe`
...
- 🧷 **CapsLock 防卡输入法** — 游戏前台保持 CapsLock 游戏态，侧栏/助手输入时临时关闭，Unicode 发送前后会同步恢复游戏态，降低 HD2 把中文输入法指令误判为游戏操作的概率
...
| 项目 | 状态 |
| --- | --- |
| Vue 面板 / 类型检查 / 前端测试 | ✅ 通过（95/95） |
| Rust 核心测试 | ✅ 通过（78/78） |
| Rust clippy / 格式检查 | ✅ 通过 |
| 前端生产构建 | ✅ 通过 |
| 记事本探针 | 待按当前版本复测 |
| HD2 窗口化填字 / 侧栏 / HUD / Esc 取消 | 待按当前版本复测 |
| NSIS 安装包 | 0.6.0 发布包将在本次构建生成 |
